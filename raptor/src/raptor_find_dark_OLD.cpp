...
int HSV_filter(int h, int s, int v, int threshold_hue,int threshold_low ,int threshold_high,int threshold_mid,int ref_hue, int ref_sat, int ref_val) {
	...
	if((diff > threshold_hue)&&(!(v<VALUE_LOW_LIM))){return 0;}//don't care
	else if((v<threshold_low)){return 1;}//abs(diff-threshold); /** If here, it has passed! */
	...
	{ // Get Blob Data 
      ...
      { // Get Blob Data 
		...
		valCent[blobs.GetNumBlobs()+i]= -1;
		  ...
		  //debug
		  blob.FillBlob(imageSmooth, cvScalar(0, 255, 0)); // This line will give you a visual marker on image for the blob if you want it for testing or something
      }    
    //
   
   cvShowImage("Dark_Detected",imageSmooth);
