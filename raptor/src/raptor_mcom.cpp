...
void raptor_mcom::lls_relmove_callback(const std_msgs::Float32::ConstPtr &msg)
{
  ...
  {
   ROS_ERROR("The low-level absmove does not accept zero or negative numbers."); 
  }
...
void raptor_mcom::find_best_light()
{
  if(!best_heading_found)
	...
	ROS_DEBUG("Light level at %d is %d",current_heading,light_levels[current_heading]);
	current_heading++;
      }
	...
	{
	  temp = light_levels[i];
	  ROS_DEBUG("LLi is %d comp to %d.",temp,val);
	...
	ROS_DEBUG("Found best heading at %d",current_heading);
      }
    }
    else
    {
